...
void rx433Handler() {  // Interrupt-Routine
  ...
  if (!rxState) {                           // wenn Eingang = Low, dann...
    ...
    if (syncBit) {                                          // wenn das SyncBit erkannt wurde, dann High-Impuls auswerten:
      ...
      if (rxCounter < 36) {                                 // Wenn noch keine 32 Bits uebertragen wurden, dann...
        rxBuffer |= (unsigned long long) dataBit << rxCounter++; // das Datenbit in den Puffer schieben und den Counter erhoehen
      } 
      if (rxCounter == 36) {         // wenn das Datentelegramm (32 Bit) vollstaendig uebertragen wurde, dann...
...
WeatherStationDataRx::WeatherStationDataRx(uint8_t dataPin) {
...
void WeatherStationDataRx::begin() {
  ...
  attachInterrupt(digitalPinToInterrupt(dataPin), rx433Handler, CHANGE);
    
  DEBUG_PRINTLN(F("WeatherStationDataRx begin"));
}

char WeatherStationDataRx::readData() {
  char newData = 0;

  if (rxOk) { // wenn die Interrupt-Routine rxOk auf true gesetzt hat, dann ist der Puffer mit den Daten gefuellt
     ...
     */
    if (!(randomID & 0x10)) { // Bit 4 der Zufalls-ID 0 ist (ist beim Wind- und Regensensor immer der Fall)
      ...
      DEBUG_PRINTLN(bState);

      if (xBits < 3) {  // xBits < 3 ist Temperatur oder Luftfeuchtigkeit
        if (calculateChecksume(0xf, false)) {
          ...
          DEBUG_PRINTLN(F("Temperatur: %d.%d °C"), (Temperature / 10), (Temperature % 10));
          DEBUG_PRINTLN(F("Luftfeuchtigkeit: %d %%"), Humidity);
      ...
      else if (xBits == 3) { // xBits = 3 ist Wind- oder Regensensor
        if ((subID == 1) || (subID == 7)) {
          if (calculateChecksume(0xf, false)) {
            if (subID == 1) { // subID = 1 ist der Windsensor (Durchschnitts-Windgeschwindigkeit)
              bState ? batteryState |= 1 : batteryState &= 0; // wenn die Batterien schwach sind, Bit 0 von batteryState auf 1 setzen
              ...
              DEBUG_PRINTLN(F("Windgeschwindigkeit (/): %d.%d m/s"), (WindSpeed / 10), (WindSpeed % 10));

              newData = 'S';
            }
            if (subID == 7) { // subID = 7 ist der Windsensor (Windrichtung und Windboen)
              bState ? batteryState |= 1 : batteryState &= 0; // wenn die Batterien schwach sind, Bit 0 von batteryState auf 1 setzen
              ...
              DEBUG_PRINTLN(F("Windrichtung: %3d Grad"), WindDirection);
              DEBUG_PRINTLN(F("Windboen: %d.%d m/s  (%d.%d km/h)"), (WindGust / 10), (WindGust % 10));
        ...
        else if (subID == 3) { // subID = 3 ist der Regensensor (absolute Regenmenge seit Batteriewechsel)
          if (calculateChecksume(0x7, true)) {
            ...
            rainVolume = ((unsigned long) (rxBuffer >> 16) & 0xffff) * 25; // die Bits 16-31 enthalten die Niederschlagsmenge
              ...
              DEBUG_PRINTLN(F("Regenmenge: %d.%d mm"), (RainVolume / 100), (RainVolume % 100));
...
bool WeatherStationDataRx::calculateChecksume(byte startValue, bool add) {
  ...
  DEBUG_PRINTLN(F(" Checksum calc=%lu read=%lu"), checksumCalc, checksum);
